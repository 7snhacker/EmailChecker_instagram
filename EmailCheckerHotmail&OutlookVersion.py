...
def check_email_availability(email, sleep_time):
    cookies = {
...
def main():
    ...
    by @7snhacker
    """)
    sleep_time = float(input("Sleep time between checks (seconds): "))
    ...
    for idx, email in enumerate(emails, 1):
        status = check_email_availability(email, sleep_time)
        if status == "available":
            print(f"[{idx}/{len(emails)}] Available: {email}")
            with open(output_file, "a") as f:
                f.write(email + "\n")
        elif status == "unavailable":
            print(f"[{idx}/{len(emails)}] Not available: {email}")
        else:
            print(f"[{idx}/{len(emails)}] Status unknown or error for {email} -> {status}")
